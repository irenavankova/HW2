...
/******************************************************************************
* Irena's comments
* The tags are wrong - tag of message sent doesn't match what receiver expects 
* and other way round
******************************************************************************/
...
if (rank == 0) {
  ...
  rc = MPI_Recv(&inmsg, 1, MPI_CHAR, source, 998, MPI_COMM_WORLD, &Stat);
  ...
  printf("Received from task %d...\n",source);
  rc = MPI_Send(&outmsg, 1, MPI_CHAR, dest, 998, MPI_COMM_WORLD);
  printf("Sent to task %d...\n",dest);
  }
